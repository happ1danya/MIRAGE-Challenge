...
class Predictor(BasePredictor):
    ...
    """
    def setup(self) -> None:
        """Load LLaVA-NeXT-Interleave-7B (4-bit) from local SSD."""
        ckpt_dir = (
            "/home/karen/Documents/Projects/mirage/"
            "LLaVA-NeXT/checkpoint/llava-interleave-qwen-7b-hf"
        )

        quant_cfg = BitsAndBytesConfig(
            load_in_4bit=True,              # fits in ~18 GB VRAM
        ...
        self.tokenizer, self.model, self.image_processor, self.context_len = (
            load_pretrained_model(
                ckpt_dir,
                model_name="llava-interleave-qwen-7b-hf",
                model_base=None,
                device_map="auto",                  # pushes layers to cuda:0
                quantization_config=quant_cfg,
                attn_implementation="flash_attention_2",  # if flash-attn 2 installed
                local_files_only=True               # never touches the Internet
            )
        )
    def predict(
        self,
